...
class LhcModelCreator(object):
    ...
    @classmethod
    def get_correction_check_script(cls, accel, outdir, corr_file="changeparameters_couple.madx", chrom=False) -> str:
        outdir = Path(outdir)
        madx_script = accel.get_base_madx_script(accel.model_dir)
        madx_script += (
            f"exec, do_twiss_monitors_and_ips(LHCB{accel.beam}, '{outdir / 'twiss_no.dat'}', 0.0);\n"
            f"call, file = '{corr_file}';\n"
            f"exec, do_twiss_monitors_and_ips(LHCB{accel.beam}, '{outdir / 'twiss_cor.dat'}', 0.0);\n")
        if chrom:
            madx_script +=(
    ...
    @classmethod
    def get_madx_script(cls, accel, outdir) -> str:  # nominal
        outdir = Path(outdir)
    ...
    @classmethod
    def prepare_run(cls, lhc_instance, output_path) -> None:
        ...
        shutil.copy(str(lib_path / LHC_MACROS), str(macros_path / LHC_MACROS))
        if lhc_instance.energy is not None:
            core = f"{int(lhc_instance.energy*1000):04d}"
            file_path: Path = lhc_instance.get_lhc_error_dir()
            shutil.copy(str(file_path / f"{core}GeV.tfs"), str(output_path / ERROR_DEFFS_TXT))
            shutil.copy(str(file_path / "b2_errors_settings" / f"beam{lhc_instance.beam}_{core}GeV.madx"),
                        str(output_path / B2_SETTINGS_MADX))
            b2_table = tfs.read(lhc_instance.get_lhc_error_dir() / f"b2_errors_beam{lhc_instance.beam}.tfs", index="NAME")
    ...
    @classmethod
    def _prepare_fullresponse(cls, lhc_instance, output_path) -> None:
        output_path = Path(output_path)
...
class LhcBestKnowledgeCreator(LhcModelCreator):
    ...
    @classmethod
    def get_madx_script(cls, accel, outdir) -> str:
        outdir = Path(outdir)
...
class LhcCouplingCreator(LhcModelCreator):
    @classmethod
    def get_madx_script(cls, lhc_instance, output_path) -> str:
